package com.powercn.grentechdriver.map;
...
public class DrivingRouteOverlay extends RouteOverlay {
    private int zoom = 11;//地图放大级别
    private float mWidth = 16;//线的宽度
    ...
    private int getcolor(String status) {
        if (status.length() > 0) {
            return ViewUnit.getColor(mContext,R.color.mapLine);
        }
...
//                .anchor(0.5f, 0.5f).icon(getDriveBitmapDescriptor()));
    ...
    private BitmapDescriptor getThroughPointBitDes() {
        return BitmapDescriptorFactory.fromResource(R.drawable.icon_end_ditu);
